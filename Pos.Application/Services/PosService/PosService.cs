...
namespace Pos.Application.Services.POSService
{
    public class PosService : IPosService
    {
        ...
        public async Task<ApiResponse<HeartBeatDto>> UpdateHeartBeatAsync(int posId, string env)
        {
            ...
            {
                // Build and log the exception
                var dto = SyncLogBuilder.Build(AlertType.Exception, ex.Message, posId: 0)
                    .WithExceptionInfo(ex)
                    .WithDomainInfo("HeartBeatService", "UpdateHeartbeat", null); // adjust module/action as needed

                await _cloudLogService.CreateCloudLog(new List<SyncLogDto> { dto }, env);

                // Return the API response with error
                return new ApiResponse<HeartBeatDto>(
        ...
        public async Task<ApiResponse<List<PosConfigurationDto>>> GetConfigurationsAsync(string env)
        {
            ...
            catch (Exception ex)
            {
                ...
                var dto = SyncLogBuilder.Build(AlertType.Exception, ex.Message, posId: 0)
                    .WithExceptionInfo(ex)
                    .WithDomainInfo("PosConfigurationService", "FetchConfigurations", null); // adjust module/action as needed
        ...
        public async Task<ApiResponse<List<PosStatus>>> InsertPosStatusAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                var dto = SyncLogBuilder.Build(AlertType.Exception, ex.Message, posId: 0)
                    .WithExceptionInfo(ex)
                    .WithDomainInfo("PosStatusService", "InsertPosStatusAsync", null); // adjust module/action as needed

                await _cloudLogService.CreateCloudLog(new List<SyncLogDto> { dto }, null);
