...
,
        ...
        private async Task ProcessHealthCheck(string id, CancellationToken token)
        {
            try
            {
                ...
                var response = await logService.GetAllUnsyncCloudAsync();
                ...
                if (response.StatusCode == ApiStatusCode.Success)
                {
                    ...
                    var apiResp = JsonSerializer.Deserialize<ApiResponse<List<LogDto>>>(postJson, JsonOpts);
        ...
        private async Task<HttpResponseMessage?> PostDataAsync(string id, List<LogDto> logDtos, CancellationToken token)
        {
            try
            {
                if (logDtos.Count == 0)
