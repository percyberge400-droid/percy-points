@echo off
SET ServiceName=POSWorker
SET ExePath=%~dp0POS.Worker.exe
...
if %errorlevel%==0 (
    ...
    sc create "%ServiceName%" binPath= "\"%ExePath%\"" start= auto DisplayName= "POS Service" >nul 2>&1
    sc description "%ServiceName%" "POS Service" >nul 2>&1
    sc start "%ServiceName%" >nul 2>&1
)
